...
TabRenderers.talent = (panel) => {
  renderTilesByGroup(panel, 'talent', { groupSubs: { 'Hiring': 'feeds the CHRO Scorecard' } });
...
TabRenderers.lnd = (panel) => {
  ...
  fillSlot('lnd', 'Learning intensity', Charts.card({
...
TabRenderers.attrition = (panel) => {
  ...
  const m = Compute.build(), ctx = Compute.ctxNow();
  fillSlot('attrition', 'headline', Charts.card({
